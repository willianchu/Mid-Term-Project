...
const insertQuiz = function(newQuiz) {
  return db.query(`INSERT
                  INTO quizzes (title, description, cut_note, time_limit, url_quiz_image, owner_id, is_unlisted)
                  VALUES ($1, $2, $3, $4, $5, $6, $7)
                  RETURNING *;`, [newQuiz.title,"This is a description of the Quiz " + newQuiz.title, 50, 60000, "http://", newQuiz.owner_id, Boolean(newQuiz.is_unlisted)]
  )
    .then(res => {
...
const updateQuiz = function(quizId, quizUpdate) {
  return db.query(`UPDATE quizzes
                   SET title=$1, description=$2, cut_note=$3, time_limit=$4, url_quiz_image=$5, owner_id=$6, is_unlisted=$7
                   WHERE id = $8
                   RETURNING *;`, [quizUpdate.title, quizUpdate.description, quizUpdate.cut_note, quizUpdate.time_limit, quizUpdate.url_quiz_image, quizUpdate.owner_id, Boolean(quizUpdate.is_unlisted) ,quizId])
    .then(res => {
...
/**
 * 3.5 - Delete a quiz from the database by given quiz id
...
/**
 * 3.6 - Update a quiz visibility from the database by given quiz id
 * @param {{}} isUnlisted An object containing all of the quiz details.
 * @return {Promise<{}>} A promise to the quiz.
 */

 const updateQuizIsUnlisted = function(quizId, isUnlisted) {
  return db.query(`UPDATE quizzes
                   SET is_unlisted=$2
                   WHERE id = $1
                   RETURNING *;`, [quizId, Boolean(isUnlisted)])
    .then(res => {
      //console.log(res.rows[0]);
      return res.rows[0];
    })
...
exports.updateQuizIsUnlisted = updateQuizIsUnlisted;
...
const insertQuestion = function(newQuestion) {
  return db
    .query(`INSERT INTO questions (question, url_picture_link, quiz_id)
            VALUES ($1, $2, $3)
            RETURNING *;`, [newQuestion.question, "http://", newQuestion.quiz_id])
    .then((result) => {
      return result.rows;
    })
    .catch((err) => {
...
/** 07 get correct answers by quiz id
...
const quizAverage = function(quizId) {
  return db
    .query(`SELECT 100 * AVG(users_results) AS average_result
...
/** 09 b get test by test id
 * Get specific test by test id.
 * @param {String} testId The id of the test.
 * @return {Promise<{}>} A promise to the test.
 */

const getTestsByTestId = function(testId) {
...
exports.getTestsByTestId = getTestsByTestId;
/** 09.1 all tests
...
const insertTest = function(newTest) {
  ...
  return db
    .query(`INSERT INTO tests (user_id, quiz_id, date_created, finish_date)
...
/**
 * 9.4 - Update a test Finish date to the database giving the test id
...
/**
 * 9.5 - Delete a test from the database by given test id
...
const getQuizScore = function(quizId) {
  return db
    .query(`SELECT users.name, 100 * SUM(
                  CASE
                      WHEN alternatives.is_correct = true THEN
