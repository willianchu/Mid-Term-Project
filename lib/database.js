...
const updateQuestion = function(questionUpdate) {
  return db.query(`UPDATE questions 
                  (question, url_picture_link, quiz_id)
                  VALUES ($1, $2, $3)
                   WHERE id = $4
                   RETURNING *;`, [questionUpdate.question, questionUpdate.url_picture_link, questionUpdate.quiz_id, questionUpdate.id])
    .then(res => {
      console.log(res.rows[0]);
      return res.rows[0];
    })
...
const insertAlternative = function(newAlternative) {
  return db
    .query(`INSERT INTO alternative (question_id, alternative, is_correct)
            VALUES ($1, $2, $3)
            RETURNING *;`, [newAlternative.question_id, newAlternative.alternative, newAlternative.is_correct])
    .then((result) => {
      return result.rows;
    })
    .catch((err) => {
 ...
 */
 ...
 const updateAlternative = function(alternativeUpdate) {
  return db.query(`UPDATE alternatives 
                  (question_id, alternative, is_correct)
                  VALUES ($1, $2, $3)
                   WHERE id = $4
                   RETURNING *;`, [alternativeUpdate.question_id, alternativeUpdate.alternative, alternativeUpdate.is_correct, alternativeUpdate.id])
    .then(res => {
      console.log(res.rows[0]);
      return res.rows[0];
    })
